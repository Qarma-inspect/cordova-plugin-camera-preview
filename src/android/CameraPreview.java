...
public class CameraPreview extends CordovaPlugin implements CameraActivity.CameraPreviewListener {
  ...
  private boolean takePictureToFile(int width, int height, int quality, String targetFileName, int orientation, boolean compressed, boolean jpg, CallbackContext callbackContext) {
    if(this.hasView(callbackContext) == false){
      return true;
    }

    takePictureCallbackContext = callbackContext;
    fragment.takePictureToFile(width, height, quality, targetFileName, orientation, compressed, jpg);
    return true;
  }
  ...
  public boolean setCameraParameterResolution(int width, int height, CallbackContext callbackContext){
    Camera mCamera = fragment.getCamera();
    if(mCamera != null){
      Camera.Parameters params = mCamera.getParameters();
      Camera.Size size = fragment.getOptimalPictureSize(width, height, params.getPreviewSize(), params.getSupportedPictureSizes());
      params.setRotation(0);
      params.setPictureSize(size.width, size.height);
      mCamera.setParameters(params);
      callbackContext.success("OK");
    }
    callbackContext.success("No camera available");
    return true;
  }
}
